"""
Spaceship Titanic — Pseudo-label Stacking
Goal: Beat LB 0.808 using semi-supervised learning.
Key ideas:
  1. Use the SAME feature set as stacking_model.py (proven LB=0.808)
  2. Pseudo-labeling: train on confident test predictions to augment training
  3. 5-seed averaging for all base models
  4. Tighter hyperparameters from Optuna tuning
"""
import pandas as pd
import numpy as np
...
print("=" * 60)
print("SPACESHIP TITANIC — Pseudo-label Stacking")
print("=" * 60)
...
def build_features(df):
    """Same feature set as stacking_model.py — proven to generalize."""
    df = df.copy()
    spend_cols = ['RoomService', 'FoodCourt', 'ShoppingMall', 'Spa', 'VRDeck']
...
for seed in seeds_r2:
    ...
    for fold, (tr_idx, va_idx) in enumerate(skf.split(X, y)):
        # Training: original fold train + ALL pseudo-labeled samples
        n_orig = len(X)
...
for alpha in np.arange(0.0, 1.05, 0.05):
    blend = alpha * stack_test_r1 + (1 - alpha) * stack_test_r2  # for test
...
print(f"\nsubmission_pseudo.csv saved (thr={best_thr:.3f}):")
...
sub_05.to_csv('submission_pseudo_05.csv', index=False)
print(f"\nsubmission_pseudo_05.csv saved (thr=0.5):")
